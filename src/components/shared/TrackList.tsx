...
const TrackList: React.FC<TrackListProps> = ({
  ...
  const handlePlay = (track: Track) => {
    ...
    const fullAudioUrl = track.audio_path.startsWith('http') ? track.audio_path : `https://wiisixdctrokfmhnrxnw.supabase.co/storage/v1/object/public/audio/${track.audio_path}`;
        ...
        <table className="w-full text-sm text-left text-zinc-400">
          <thead className="text-xs text-zinc-700 uppercase bg-zinc-900 sticky top-0 z-10">
            <tr>
              ...
              {tracks.some(track => track.album_name) && <th scope="col" className="px-6 py-3">Album</th>}
              <th scope="col" className="px-6 py-3">Actions</th>
            </tr>
          </thead>
          <tbody>
            {tracks.map((track, index) => <tr key={track.id} className="bg-zinc-800 border-b border-zinc-700 hover:bg-zinc-700 h-14">
                <td className="px-2 py-4 text-center">
                  <button onClick={() => handlePlay(track)} className="text-zinc-400 hover:text-white" aria-label={`Play ${track.title}`} data-track-id={track.id}>
                    <Play size={15} />
                  </button>
                </td>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center space-x-4">
                    ...
                    {track.audio_path && <button onClick={() => handleDownload(track)} className={`text-zinc-400 hover:text-white ${downloadingTrack === track.id ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={downloadingTrack === track.id} aria-label={`Download ${track.title}`}>
                        {downloadingTrack === track.id ? <Loader2 className="h-5 w-5 animate-spin" /> : <Download size={15} />}
                      </button>}
                    {albumId && !canEdit && <AddToPlaylistButton trackId={track.id} albumName={track.album_name} />}
                    {canEdit && onRemoveTrack && <button onClick={() => handleRemoveFromPlaylist(track.id)} className={`text-zinc-400 hover:text-rose-500 ${loadingRemove === track.id ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={loadingRemove === track.id}>
                        {loadingRemove === track.id ? <Loader2 className="h-5 w-5 animate-spin" /> : <X size={15} />}
                      </button>}
                  </div>
                </td>
              </tr>)}
