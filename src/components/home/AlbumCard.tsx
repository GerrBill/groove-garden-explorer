...
const AlbumCard: React.FC<AlbumCardProps> = ({ 
  ...
  if (id) {
    return (
      <Link 
        to={`/album/${id}`} 
        className={`${sizeClasses[size]} block bg-spotify-card bg-opacity-40 p-2 rounded-md hover:bg-opacity-70 transition-all group cursor-pointer`}
  ...
  return (
    <div className={`${sizeClasses[size]} bg-spotify-card bg-opacity-40 p-2 rounded-md hover:bg-opacity-70 transition-all group cursor-pointer`}>
      {content}
    </div>
  );
};
